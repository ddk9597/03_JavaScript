...
/* 
    로딩, 이벤트 발생 등 시점에 따라서 선택되는 요소가 달라짐
    첫 화면부터 계속 있는 요소는 전역변수로 설정,
    달라지는 요소는 지역변수로 설정한다.
*/

/* 
    새롭게 만들어지는 요소(동적 추가 요소)에 이벤트를 추가하는 법
    대상 요소가 만들어지는 시점에 이벤트 추가 코드를 작성한다.
    미리 만들면 현재 생성되지 않은 없는 요소에 적용시키는 코드이며 
    동적추가요소에는 적용되지 않으므로 쓸모없게됨
*/
