...
inputPhone.addEventListener("input", e=>{
    ...
    // 입력한 값이 없을 경우
    if(value.trim().length == 0){
        phoneResult.textContent = ""; // 메세지 출력 내용 삭제
        phoneResult.classList.remove("check", "uncheck") ; // 클래스 제거(2개 다)
    ...
    const regExp = /^01[0,1,6,7,9]-\d{3,4}-\d{4}$/

    if(regExp.test(value)){ // 입력 받은 값이 정규식과 일치한다면
    phoneResult.textContent = " 유효한 번호 형식입니다" ;
    phoneResult.classList.add("check");         // check클래스 추가
    phoneResult.classList.remove("uncheck");    // uncheck 클래스 제거
    }else{
        phoneResult.textContent = " 번호가 유효하지 않습니다" ;
        phoneResult.classList.add("uncheck");         // uncheck클래스 추가
        phoneResult.classList.remove("check");    // check 클래스 제거
    }/^01[0,1,6,7,9]-\d{3,4}-\d{4}$/
